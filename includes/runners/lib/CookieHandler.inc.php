...
Class CookieHandler{
    ...
    function SetCookie()
    {
        ...
        if (isset($form['location']))
        {
            ...
            if (count($forms) == '3')
            {
                list($form['location'],$form['state'],$form['country']) = $forms;
            }
            else
            {
                list($form['location'],$form['state']) = $forms;
            }
        }
        
        $location = (isset($form['location'])) ? trim($form['location']) : $this->conf['default Settings']['location'];
