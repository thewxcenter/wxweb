<?php
  ...
  class RawMETAR extends WxWebAPI
  {
  			...
  			{
  						$this->runconf = parse_ini_file(CONFIGS .'/metar.conf.php', TRUE);
...
$values=array("icao"=>$icao);$turl = preg_replace_callback('/\{(\w+)\}/', function($match) use ($values) {if (isset($values[$match[1]])) {	return $values[$match[1]];} else {
						return $match[0];   }},$url);
  $cachepath= preg_replace_callback('/\{(\w+)\}/', function($match) use ($values) {
if (isset($values[$match[1]])) {	return $values[$match[1]];} else {
						return $match[0];   }},$this->runconf['metar_cache']);
                        ...
                        $url = $this->runconf['metar_url'] . $this->runconf['metar_url_tail'];
  						...
  						$turl = preg_replace_callback('/\{(\w+)\}/', function($match) use ($values) {if (isset($values[$match[1]])) {	return $values[$match[1]];} else {
						return $match[0];   }},$url);
  $cachepath= preg_replace_callback('/\{(\w+)\}/', function($match) use ($values) {
if (isset($values[$match[1]])) {	return $values[$match[1]];} else {
