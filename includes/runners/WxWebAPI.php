  ...
  class WxWebAPI
  {
  			...
  			function __construct($debug, $form, $conf, $mysql, $smarty)
  			{
                        ...
                        $this->smarty->assign('wxwebapiversion' , $this->version);
                         $this->smarty->assign('phpver' , phpversion());
  			...
  			{//($form, $database,$fetch,$var,$debug,$mysql,$smarty)
...
//print_r($form);
  						...
  						if (!empty($form['location']) && !empty($form['state'])&& !preg_match('/\d\d\d\d\d/', $form['location']))
  						{
  									$fcity = $form['location'];
  									$fstate = $form['state'];
  						...
  						} elseif (!isset($form['location']) || !isset($form['lat']) || !isset($form['location']))
  						{
